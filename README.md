...
- 支持阀位点击持续吹气：点击阀位开始按单次循环持续吹气，再次点击停止，点击其他阀位会按“关闭阀开关、重新打开阀开关、发送目标阀位”的顺序切换。
- 支持单阀循环递增：切换到该触发模式时下发触发模式、吹气次数和吹气间隔参数，点击开始或手动选择阀位时不再重复下发。每次切换阀位均依次发送 `AA 00 00 00`、`AA 00 00 01` 和目标阀位命令；两个阀开关命令均等待同地址回包并留出 50 ms 状态稳定时间后再继续。目标阀位命令实际写入串口后开始计时，显示阀位完成后回到 1 号阀循环。
- 支持读取固件版本、读取通道异常状态。
...
- 软件控制持续吹气期间以当前目标阀为唯一绿色指示，忽略该通道延迟到达的旧阀状态脉冲，避免多个阀位同时显示为工作状态。
