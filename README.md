...
- 支持阀位点击持续吹气：点击阀位开始按单次循环持续吹气，再次点击停止，点击其他阀位会先关闭当前阀再切换。
- 支持单阀循环递增：在测试模式的触发模式中选择该模式，点击同一行的开始按钮后，所有已配置通道分别从 1 号阀开始。软件确认旧阀关闭后再开启下一阀，并从开启回包确认后开始计时，显示阀位完成后回到 1 号阀循环。
- 支持读取固件版本、读取通道异常状态。
...
- 测试模式显示 `单阀测试时间`，单位为 `ms`，用于控制单阀循环递增时每个阀位的工作时长。
...
- 收到阀动作回包时，对应指示灯显示绿色。
- 阀位点击持续吹气和单阀循环递增期间，当前工作阀保持绿色；切换阀位时软件会先关闭旧阀，再开启新阀。
